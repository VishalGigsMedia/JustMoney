...
class TCRepository {
    ...
    fun getTC(context: Context, api: API): MutableLiveData<FaqModel> {
        val mutableLiveData: MutableLiveData<FaqModel> = MutableLiveData()
        if (DefaultHelper.isOnline()) {
            val preferenceHelper = PreferenceHelper(context)
            api.getTC(preferenceHelper.getJwtToken()).enqueue(object : Callback<FaqModel> {
    ...
    fun getPP(context: Context, api: API): MutableLiveData<FaqModel> {
        val mutableLiveData: MutableLiveData<FaqModel> = MutableLiveData()
        if (DefaultHelper.isOnline()) {
            val preferenceHelper = PreferenceHelper(context)
            api.getPP(preferenceHelper.getJwtToken()).enqueue(object : Callback<FaqModel> {
                override fun onResponse(call: Call<FaqModel>, response: Response<FaqModel>) {
                    gson = gsonBuilder.create()
                    val json = Gson().toJson(response.body())
                    faqModel = gson?.fromJson(json, FaqModel::class.java)
